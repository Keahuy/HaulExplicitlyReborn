...
public class WorkGiver_HaulExplicitly : WorkGiver_Scanner
{
    ...
    public override Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
    {
        ...
        Data_DesignatorHaulExplicitly posting = GameComponent_HaulExplicitly.GetManager(t).PostingWithItem(t);
        if (posting == null)
            return null;
        int spaceRequest = AmountPawnWantsToPickUp(pawn, t, posting); // 需要搬运的物品数
    ...
    public static int AmountPawnWantsToPickUp(Pawn p, Thing t, Data_DesignatorHaulExplicitly posting)
    {
        return Mathf.Min(posting.GetRecordWhichWithItem(t).RemainingToHaul(), p.carryTracker.AvailableStackSpace(t.def), t.stackCount);
    }
}